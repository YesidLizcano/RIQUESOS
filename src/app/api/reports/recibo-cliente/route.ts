...
import { generateReciboClientePdf } from '@/infrastructure/pdf/templates/recibo-venta';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const buffer = await generateReciboClientePdf(venta, ventaRecord.sede?.nombre ?? undefined);
    const date = new Date().toISOString().slice(0, 10);

    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `inline; filename="Recibo_Cliente_${date}.pdf"`,
      },
    });
  } catch (error) {
    console.error('Error generating Recibo Cliente PDF:', error);
